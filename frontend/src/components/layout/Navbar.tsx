...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="max-w-[1340px] mx-auto">
          ...
          <div
            className={`hidden lg:flex items-center justify-between px-6 py-2.5 rounded-[26px] transition-all duration-300 ${
              scrolled
                ? 'bg-white/95 backdrop-blur-2xl border border-slate-200 shadow-xl shadow-slate-950/5'
                : 'bg-white/95 backdrop-blur-xl border border-slate-200/90 shadow-lg shadow-slate-950/5'
            }`}
          >
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center space-x-2.5 px-3.5 py-1.5 rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200/80 transition-all text-left group cursor-pointer shadow-2xs"
                >
                  <div className="w-6 h-6 rounded-xl bg-slate-900 text-white flex items-center justify-center shrink-0 shadow-2xs">
                ...
                {branchDropdownOpen && (
                  <div className="absolute left-0 mt-2 w-64 bg-white rounded-2xl p-2 border border-slate-200 shadow-2xl space-y-1 text-xs z-50 animate-in fade-in slide-in-from-top-2 max-h-72 overflow-y-auto">
                    <div className="px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Select Active Branch</div>
                    {branches.map((branch) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center space-x-2.5 truncate">
                            <MapPin className={`w-3.5 h-3.5 shrink-0 ${isSelected ? 'text-rose-400' : 'text-slate-400'}`} />
                            <div className="truncate">
                              <p className="truncate leading-tight font-bold">{branch.name}</p>
                              <p className={`text-[9px] truncate leading-tight ${isSelected ? 'text-slate-300' : 'text-slate-500'}`}>{branch.city}, {branch.state}</p>
                            </div>
                          </div>
                          {isSelected && <span className="w-2 h-2 rounded-full bg-rose-400 shrink-0 ml-1" />}
                        </button>
                      );
                    })}
                  </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="relative">
                <button
                  ...
                  className="relative p-2 rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200/80 text-slate-700 transition-all cursor-pointer shadow-2xs"
                  title="Live Notifications"
                >
                  <Bell className="w-4 h-4" />
                  {unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 min-w-[17px] h-[17px] px-1 bg-gradient-to-r from-rose-500 to-purple-600 text-white text-[9px] font-black rounded-full flex items-center justify-center shadow-xs animate-pulse ring-2 ring-white">
              ...
              {user ? (
                <div className="relative flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-2 px-3 py-1.5 rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200/80 transition-all text-xs font-extrabold text-slate-900 cursor-pointer shadow-2xs"
                  >
                    <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-purple-600 to-rose-500 text-white flex items-center justify-center font-black text-xs shadow-2xs">
                  ...
                  <Link
                    ...
                    className="flex items-center space-x-1.5 px-3 py-1.5 rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200/80 transition-all text-slate-900 text-xs font-black cursor-pointer shadow-2xs"
                  >
                    <Wallet className="w-3.5 h-3.5 text-emerald-600" />
                    <span className="text-emerald-700 font-extrabold">₹{Number(user.walletBalance || 0).toLocaleString()}</span>
                  </Link>

                  {/* User Dropdown Menu */}
                  {userDropdownOpen && (
                    <div className="absolute right-0 top-12 w-60 bg-white rounded-2xl p-2 border border-slate-200 shadow-2xl space-y-1 text-xs z-50 animate-in fade-in slide-in-from-top-2">
                      <div className="px-3 py-2 border-b border-slate-100 bg-slate-50 rounded-xl mb-1">
                ...
                <div className="flex items-center space-x-2">
                  <Link
                    href="/login"
                    className="px-4 py-2 text-xs font-extrabold rounded-2xl border border-slate-200 bg-white hover:bg-slate-50 transition-all text-slate-700 hover:text-slate-900 shadow-2xs"
                  >
                    Sign In
                  </Link>
                  ...
                  <Link
                    href="/register"
                    className="px-4.5 py-2 text-xs font-extrabold rounded-2xl text-white bg-slate-900 hover:bg-slate-800 transition-all shadow-md flex items-center space-x-1.5"
                  >
          ...
          <div className="lg:hidden relative">
            ...
            {mobileBranchOpen && (
              <div className="absolute top-12 left-1/2 -translate-x-1/2 w-[92vw] max-w-sm bg-white rounded-2xl p-2.5 border border-slate-200 shadow-2xl space-y-1.5 text-xs z-50 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="space-y-1 max-h-56 overflow-y-auto">
                  {branches.map((branch) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center space-x-2.5 truncate">
                          <div className={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 ${isSelected ? 'bg-slate-800 text-rose-400' : 'bg-slate-100 text-slate-500'}`}>
                            <MapPin className="w-3.5 h-3.5" />
                          </div>
                          <div className="truncate">
                            <p className="truncate font-bold text-xs">{branch.name}</p>
                            <p className={`text-[10px] truncate ${isSelected ? 'text-slate-300' : 'text-slate-400'}`}>{branch.city}, {branch.state}</p>
                          </div>
                        </div>
                        {isSelected && <span className="w-2 h-2 rounded-full bg-rose-400 shrink-0 ml-1.5" />}
                      </button>
                    );
                  })}
                </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-x-0 top-16 z-40 bg-white border-b border-slate-200/90 px-4 pt-4 pb-8 space-y-4 shadow-2xl animate-in slide-in-from-top-3 max-h-[85vh] overflow-y-auto">
          {/* Mobile Location Selector Bar */}
          <div className="space-y-2">
            <button
              type="button"
              onClick={() => setMobileBranchOpen(!mobileBranchOpen)}
              className="w-full flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-all text-left cursor-pointer shadow-2xs"
            >
              <div className="flex items-center space-x-2.5">
                <div className="w-7 h-7 rounded-xl bg-slate-900 text-rose-400 flex items-center justify-center shrink-0">
            ...
            {mobileBranchOpen && (
              <div className="p-2 rounded-2xl bg-white border border-slate-200 shadow-md space-y-1 text-xs">
                <div className="px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">Select Active Branch</div>
                {branches.map((branch) => {
                  const isSelected = activeBranch?.id === branch.id;
                  return (
                    <button
                    ...
                    >
                      <div className="flex items-center space-x-2 truncate">
                        ...
                        <div className="truncate">
                          ...
                          <p className={`text-[9px] truncate ${isSelected ? 'text-slate-300' : 'text-slate-500'}`}>{branch.city}, {branch.state}</p>
                        </div>
                      </div>
                      {isSelected && <span className="w-2 h-2 rounded-full bg-rose-400 shrink-0 ml-1" />}
                    </button>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {navLinks.map((link, idx) => {
              ...
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${isActive ? 'bg-slate-800 text-rose-400' : 'bg-white text-slate-600 border border-slate-200'}`}>
            ...
            {user && (
              ...
              >
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${pathname === '/bookings' ? 'bg-slate-800 text-rose-400' : 'bg-white text-slate-600 border border-slate-200'}`}>
          ...
          <div className="pt-3 border-t border-slate-100 space-y-2">
            {user ? (
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-2">
                ...
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-white border border-slate-200 text-slate-900 text-xs font-black">
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <Link
                    href="/dashboard"
                    onClick={() => setMobileMenuOpen(false)}
                    className="py-2 text-center text-xs font-extrabold rounded-xl bg-slate-900 text-white shadow-xs"
                  >
                    Dashboard
                  </Link>
                  <button
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Link
                  href="/register"
                  onClick={() => setMobileMenuOpen(false)}
                  className="py-2.5 text-center text-xs font-extrabold rounded-xl text-white bg-slate-900 shadow-md"
                >
                  Register Free
                </Link>
              </div>
